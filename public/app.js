...
function statusClass(status){return ({'Ожидает оплаты':'pending','Оплачен':'paid','В работе':'working','Завершён':'done','Отменён':'cancelled'})[status]||'';}
function statusIcon(status){return ({'Ожидает оплаты':'🟠','Оплачен':'🔵','В работе':'🟡','Завершён':'🟢','Отменён':'🔴'})[status]||'⚪';}
function formatDate(value){const d=new Date(value||0);return Number.isNaN(d.getTime())?'':d.toLocaleString('ru-RU',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'});}
async function loadMyOrders(){const list=$('ordersList');if(!list)return;list.innerHTML='<div class="loading-card"><span class="spinner"></span><span>Загружаем заказы…</span></div>';try{const r=await fetch('/api/my-orders',{headers:{'X-Telegram-Init-Data':tg?.initData||''}});const d=await r.json();if(!r.ok||!d.ok)throw Error(d.error||'Откройте Mini App из Telegram');if(!d.orders.length){list.innerHTML='<div class="empty-card"><div class="empty-icon">🛒</div><h3>Заказов пока нет</h3><p>Оформите первый буст на главной странице.</p><button class="secondary-btn" data-page-jump="home">Перейти к заказу</button></div>';return;}list.innerHTML=d.orders.map(o=>{const modeName=o.mode==='allies'?'Allies':'MM';const accessName=o.access==='with'?'Со входом':'Без входа';const price=o.prices?.[o.payment]??o.prices?.rub??0;const currency=o.payment==='stars'?'Stars':o.payment==='gold'?'Gold':'₽';return `<article class="order-history-card"><div class="order-history-top"><div><b>${escapeHtml(o.id)}</b><small>${formatDate(o.updatedAt||o.createdAt)}</small></div><span class="status-pill ${statusClass(o.status)}">${statusIcon(o.status)} ${escapeHtml(o.status)}</span></div><div class="order-route"><span>${escapeHtml(o.fromRank)}</span><i>→</i><strong>${escapeHtml(o.toRank)}</strong></div><div class="order-meta"><span>🎮 ${modeName}</span><span>🔐 ${accessName}</span><span>💳 ${price} ${currency}</span></div>${o.booster?`<div class="booster-line">👤 Бустер: <b>${escapeHtml(o.booster)}</b></div>`:''}${o.status==='Завершён'?'<div class="done-line">✓ Заказ завершён — спасибо за выбор Nova Boost</div>':''}</article>`;}).join('');}catch(e){list.innerHTML=`<div class="empty-card"><div class="empty-icon">🔔</div><h3>Не удалось загрузить заказы</h3><p>${escapeHtml(e.message||'Откройте Mini App из Telegram')}</p></div>`;}}
function jumpTo(page){const item=document.querySelector(`.nav-item[data-page="${page}"]`);if(item){item.click();return;}}
...
document.addEventListener('click',e=>{const jump=e.target.closest('[data-page-jump]');if(jump){e.preventDefault();jumpTo(jump.dataset.pageJump);}});
$('refreshOrders')?.addEventListener('click',()=>{haptic();loadMyOrders();});
...
update();loadReviews();
if(tg?.initData)setTimeout(loadMyOrders,1200);
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'&&$('orders')?.classList.contains('active'))loadMyOrders();});
