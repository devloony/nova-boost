...
async function createOrder(){
  ...
  // Надёжный fallback: Telegram user также находится внутри initData.
  try{
    const raw=new URLSearchParams(tg?.initData||'').get('user');
    if(raw){const parsed=JSON.parse(raw); if(parsed&&typeof parsed==='object') user={...parsed,...user};}
  }catch{}
  const username=String(user.username||'').trim().replace(/^@+/,'');
  orderBtn.disabled=true; orderBtn.querySelector('b').textContent='СОЗДАНИЕ ЗАКАЗА...'; message.style.display='none';
  try{
    const response=await fetch('/api/order',{method:'POST',headers:{'Content-Type':'application/json','X-Telegram-Init-Data':tg?.initData||''},body:JSON.stringify({fromRank,toRank,mode,access,payment,promoCode,user:{id:user.id||null,username:username||null,first_name:user.first_name||null,last_name:user.last_name||null},initData:tg?.initData||''})});
