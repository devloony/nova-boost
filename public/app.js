...
async function createOrder(){
  ...
  // Личность пользователя определяет сервер по валидированному initData.
  orderBtn.disabled=true; orderBtn.querySelector('b').textContent='СОЗДАНИЕ ЗАКАЗА...'; message.style.display='none';
  try{
    const response=await fetch('/api/order',{method:'POST',headers:{'Content-Type':'application/json','X-Telegram-Init-Data':tg?.initData||''},body:JSON.stringify({fromRank,toRank,mode,access,payment,promoCode,initData:tg?.initData||''})});
