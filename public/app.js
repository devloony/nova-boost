...
let mode = "allies", access = "with", payment = "rub", fromRank = null, toRank = null, reviewStars = 5;
...
function update(){
  ...
  const p=priceFor();
  if(p){selection.textContent=`${mode==='allies'?'Allies':'MM'} · ${fromRank} → ${toRank}`;priceLines.innerHTML=`<span>${p[0]} ₽</span><span>${p[1]} Gold</span><span>${p[2]} Stars</span>`;orderBtn.disabled=false;}
...
async function createOrder(){
  ...
  try{
    const response=await fetch('/api/order',{method:'POST',headers:{'Content-Type':'application/json','X-Telegram-Init-Data':tg?.initData||''},body:JSON.stringify({fromRank,toRank,mode,access,payment,user:{id:user.id,username:user.username,first_name:user.first_name},initData:tg?.initData||''})});
...
document.querySelectorAll('.nav-item').forEach(item=>item.addEventListener('click',()=>{const target=item.dataset.page;document.querySelectorAll('.page').forEach(p=>p.classList.toggle('active',p.id===target));document.querySelectorAll('.nav-item').forEach(n=>n.classList.toggle('active',n===item));if(target==='reviews')loadReviews();window.scrollTo({top:0,behavior:'smooth'});}));
...
async function loadReviews(){try{const r=await fetch('/api/reviews');const d=await r.json();const list=$('reviewsList');if(!d.reviews.length){list.innerHTML='<div class="empty-card"><div class="empty-icon">★</div><h3>Пока нет отзывов</h3><p>Оставьте первый отзыв после заказа.</p></div>';return}list.innerHTML=d.reviews.map(x=>`<div class="review-card"><div class="review-head"><span class="review-name">${escapeHtml(x.name)}</span><span class="review-stars">${'★'.repeat(x.stars)}${'☆'.repeat(5-x.stars)}</span></div><div class="review-text">${escapeHtml(x.text)}</div></div>`).join('')}catch{}}
$('reviewBtn').addEventListener('click',async()=>{const name=$('reviewName').value.trim(),text=$('reviewText').value.trim();if(!name||!text)return showMessage('Заполните имя и текст отзыва.');try{const r=await fetch('/api/reviews',{method:'POST',headers:{'Content-Type':'application/json','X-Telegram-Init-Data':tg?.initData||''},body:JSON.stringify({name,text,stars:reviewStars,user:tg?.initDataUnsafe?.user||{},initData:tg?.initData||''})});const d=await r.json();if(!d.ok)throw Error(d.error||'Ошибка');$('reviewName').value='';$('reviewText').value='';showMessage('Спасибо! Отзыв отправлен.');loadReviews()}catch(e){showMessage(e.message||'Не удалось отправить отзыв.')}});
