...
app.post("/api/order",async(req,res)=>{
 try{
  // Для RUB/Gold заказ можно создать даже если Telegram initData
  // не пришёл (например, Mini App открыт через обычную ссылку).
  // Для Stars initData обязателен, потому что оплата проходит через Telegram.
  const telegramUser = userFromReq(req);
  const bodyUser = req.body?.user && typeof req.body.user === "object"
    ? req.body.user
    : {};
  const payment = String(req.body?.payment || "").toLowerCase();

  if(payment === "stars" && !telegramUser){
    return res.status(401).json({
      ok:false,
      error:"Для оплаты Stars откройте Mini App из Telegram и попробуйте снова."
    });
  }

  // Identity comes from validated Telegram initData when available.
  // Username is additionally taken from the Mini App payload as a fallback
  // because Telegram user objects may omit username in some launch contexts.
  // For display only, also try to recover the user object directly from initData.
  // Ownership/security still relies on validated initData above.
  let rawInitUser = null;
  try {
    const rawParams = new URLSearchParams(req.headers["x-telegram-init-data"] || req.body?.initData || "");
    rawInitUser = JSON.parse(rawParams.get("user") || "null");
  } catch {}

  const displayUser = {
    ...(rawInitUser && typeof rawInitUser === "object" ? rawInitUser : {}),
    ...(bodyUser && typeof bodyUser === "object" ? bodyUser : {}),
    ...(telegramUser && typeof telegramUser === "object" ? telegramUser : {})
  };
  const normalizedUsername = String(
    telegramUser?.username || bodyUser.username || rawInitUser?.username || ""
  ).trim().replace(/^@+/, "");
  const orderUser = {
    id: telegramUser?.id || bodyUser.id || rawInitUser?.id || null,
    username: normalizedUsername || null,
    first_name: telegramUser?.first_name || bodyUser.first_name || rawInitUser?.first_name || null,
    last_name: telegramUser?.last_name || bodyUser.last_name || rawInitUser?.last_name || null
  };

  const order=makeOrder(req.body, orderUser);
  // Resolve the username directly through Telegram as a final fallback.
  if (order.user?.id && !order.user?.username) {
   try {
    const chat = await tgApi("getChat", {chat_id: order.user.id});
    const tgUsername = String(chat?.username || "").trim().replace(/^@+/, "");
    if (tgUsername) order.user.username = tgUsername;
   } catch(e) { console.error("Username lookup error", e.message); }
  }
...
app.post("/api/admin/review-delete",(req,res)=>{
 if(!isAdmin(req))return res.status(403).json({ok:false,error:"Доступ запрещён"});
 const id=String(req.body.id||"");
 const index=reviews.findIndex(r=>String(r.id)===id);
 if(index<0)return res.status(404).json({ok:false,error:"Отзыв не найден"});
 if(reviews[index].approved!==false)return res.status(400).json({ok:false,error:"Можно удалять только неопубликованные отзывы"});
...
bot.on("successful_payment",async msg=>{const sp=msg.successful_payment;let payload={};try{payload=JSON.parse(sp.invoice_payload)}catch{};const o=orders.find(x=>x.id===payload.orderId);if(o){o.status="Оплачен";o.updatedAt=new Date().toISOString();o.telegramPaymentChargeId=sp.telegram_payment_charge_id;o.paidAt=new Date().toISOString();save(ORDERS_FILE,orders);await notifyUser(o.user?.id,`💳 Nova Boost

Оплата заказа ${o.id} получена.
${orderLabel(o)}

Статус: Оплачен`);try{await bot.sendMessage(ADMIN_ID,`💳 ОПЛАТА ПОЛУЧЕНА\nЗаказ: ${o.id}\n${o.fromRank} → ${o.toRank}\nСумма: ${sp.total_amount} Stars\nПользователь: ${msg.from?.username?`@${msg.from.username}`:msg.from?.first_name||msg.from?.id}`)}catch(e){console.error(e.message)}}});
