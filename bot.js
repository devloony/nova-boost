...
function validateInitData(initData){
 ...
 const secret=crypto.createHmac("sha256",token).update("WebAppData").digest();const check=crypto.createHmac("sha256",secret).update(pairs.join("\n")).digest("hex");
 if(check.length!==hash.length||!crypto.timingSafeEqual(Buffer.from(check),Buffer.from(hash)))return null;
 const authDate=Number(params.get("auth_date")||0);if(!authDate||Date.now()/1000-authDate>86400)return null;try{return JSON.parse(params.get("user")||"null")}catch{return null}
}
...
app.post("/api/order",async(req,res)=>{
 try{
  const order=makeOrder(req.body);let invoiceLink=null;
...
bot.on("pre_checkout_query",async q=>{try{const payload=JSON.parse(q.invoice_payload);const o=orders.find(x=>x.id===payload.orderId);if(!o||o.payment!=="stars"||o.status!=="Ожидает оплату")return bot.answerPreCheckoutQuery(q.id,false,{error_message:"Заказ не найден или уже недействителен."});await bot.answerPreCheckoutQuery(q.id,true)}catch(e){console.error(e);try{await bot.answerPreCheckoutQuery(q.id,false,{error_message:"Не удалось проверить заказ."})}catch{}}});
