...
app.post("/api/order",async(req,res)=>{
 try{
  // Пользователь определяется ТОЛЬКО сервером из валидированного
  // Telegram Web App initData. Данные user из браузера больше не используются
  // как источник личности пользователя.
  const telegramUser = userFromReq(req);
  if(!telegramUser){
    return res.status(401).json({
      ok:false,
      error:"Откройте Nova Boost через Telegram Mini App и попробуйте снова."
    });
  }

  const normalizedUsername = String(telegramUser.username || "")
    .trim().replace(/^@+/, "");
  const orderUser = {
    id: telegramUser.id,
...
Статус: ${order.status}`);
  try{
   const hasUsername=Boolean(order.user?.username);
   const userText=hasUsername?`@${String(order.user.username).replace(/^@+/,'')}`:'Открыть профиль';
   const markup=!hasUsername&&order.user?.id?{inline_keyboard:[[{text:'👤 Открыть профиль',url:`tg://user?id=${order.user.id}`}]]}:undefined;
   await tgApi("sendMessage",{chat_id:ADMIN_ID,parse_mode:"HTML",text:`🆕 НОВЫЙ ЗАКАЗ ${order.id}\n\nРежим: ${order.mode==='allies'?'Allies':'MM'}\nЗвания: ${order.fromRank} → ${order.toRank}\nДоступ: ${order.access==='with'?'Со входом':'Без входа'}\nОплата: ${order.payment}\nЦена: ${order.prices.rub} ₽ / ${order.prices.gold} Gold / ${order.prices.stars} Stars${order.promoCode?`\nПромокод: ${order.promoCode} (-${order.discountPercent}%)`:''}\nПользователь: ${userText}\n\nСтатус: ${order.status}`,...(markup?{reply_markup:markup}:{})});
